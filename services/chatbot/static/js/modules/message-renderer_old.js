...
export class MessageRenderer {
    constructor() {
        this.modelNames = {
            ...
            'deepseek-reasoner': '≡ƒºá DeepSeek R1',
            'qwen': 'Qwen Turbo',
            'bloomvn': 'BloomVN-8B',
        ...
        this.contextNames = {
            'casual': '≡ƒÆ¼ Casual Chat',
            'psychological': '≡ƒºÿ Psychology',
            'lifestyle': '≡ƒîƒ Lifestyle',
            'programming': '≡ƒÆ╗ Programming',
            'creative': '≡ƒÄ¿ Creative',
            'research': '≡ƒö¼ Research'
        };
    ...
    _setReasoningBlockExpanded(block, expanded) {
        ...
        if (icon) icon.textContent = expanded ? 'Γû╝' : 'Γû╢';
    }
    ...
    _createReasoningBlock(stepText, tid = '_default', expanded = false) {
        ...
        const labelText = tidMatch
            ? `≡ƒöì Direction ${parseInt(tidMatch[2], 10) + 1} (round ${parseInt(tidMatch[1], 10) + 1})`
            : '≡ƒöì Reasoning';
    ...
    _initSelectAndReply() {
        ...
        this._replyPopup.innerHTML = '<button class="select-reply-btn"><i data-lucide="reply" class="lucide" style="width:14px;height:14px"></i> Trß║ú lß╗¥i ─æoß║ín n├áy</button>';
        ...
        // On mouseup anywhere ΓÇö show/hide the floating button
    ...
    setQuotedContext(text) {
        ...
        const truncated = text.length > 200 ? text.substring(0, 200) + 'ΓÇª' : text;
    ...
    addMessage(chatContainer, content, isUser, model, context, timestamp, thinkingProcess = null, customPromptUsed = false, agentConfig = null, attachedFiles = null) {
        ...
        if (isUser && attachedFiles && attachedFiles.length > 0) {
            ...
            attachedFiles.forEach(file => {
                ...
                const meta = file.tableData
                    ? `${file.tableData.rows.length} h├áng ┬╖ ${file.tableData.headers.length} cß╗Öt`
        ...
        if (!isUser && model && context) {
            ...
            let configIndicator = '≡ƒô¥ Base Prompt';
            let configDetails = '';
            ...
            if (agentConfig && agentConfig.enabled) {
                ...
                if (thinkingMode === 'advanced') {
                    configIndicator = '≡ƒÜÇ Config Agent (Advanced)';
                    configDetails = ` ΓÇó T:${temp}`;
                } else if (thinkingMode === 'on') {
                    configIndicator = 'ΓÜí Config Agent (Thinking)';
                    configDetails = ` ΓÇó T:${temp}`;
                } else {
                    configIndicator = 'ΓÜí Config Agent';
                    configDetails = ` ΓÇó T:${temp}`;
                }
            } else if (customPromptUsed) {
                configIndicator = '≡ƒ¢á∩╕Å Custom Prompt';
            }
            
            infoDiv.textContent = `${this.modelNames[model] || model} ΓÇó ${this.contextNames[context] || context} ΓÇó ${configIndicator}${configDetails}`;
    ...
    addFileMessage(chatContainer, files, timestamp) {
        ...
        headerDiv.innerHTML = `≡ƒôÄ <strong>─É├ú tß║úi l├¬n ${files.length} file${files.length > 1 ? 's' : ''}</strong>`;
    ...
    getFileIcon(type, name) {
        ...
        if (type.startsWith('audio/')) return '≡ƒÄ╡';
        if (name.endsWith('.csv') || name.endsWith('.tsv')) return '≡ƒôè';
        if (type === 'application/pdf') return '≡ƒôò';
        if (type === 'application/msword' || type.includes('wordprocessing')) return '≡ƒôÿ';
        if (type.includes('spreadsheet') || name.endsWith('.xlsx')) return '≡ƒôè';
        if (type === 'application/json') return '≡ƒôï';
        if (name.endsWith('.py')) return '≡ƒÉì';
        if (name.endsWith('.js')) return '≡ƒô£';
        if (name.endsWith('.html')) return '≡ƒîÉ';
        if (name.endsWith('.css')) return '≡ƒÄ¿';
        if (type.startsWith('text/')) return '≡ƒôä';
        return '≡ƒôÄ';
    }
    ...
    _buildInlineTablePreview(tableData, filename, maxRows = 10) {
        ...
        headerBar.innerHTML = `
            <span class="inline-table-title">≡ƒôè ${this.escapeHtml(filename)}</span>
            <span class="inline-table-stats">${rows.length} h├áng ┬╖ ${headers.length} cß╗Öt</span>
        `;
        ...
        tbody.innerHTML = displayRows.map(row =>
            `<tr>${headers.map(h => {
                ...
                const display = str.length > 60 ? str.substring(0, 60) + 'ΓÇª' : str;
        ...
        if (rows.length > maxRows) {
            ...
            footer.innerHTML = `<span class="inline-table-truncated">Hiß╗ân thß╗ï ${maxRows}/${rows.length} h├áng</span>`;
            const viewBtn = document.createElement('button');
            viewBtn.className = 'inline-table-view-btn';
            viewBtn.textContent = '≡ƒôï Xem ─æß║ºy ─æß╗º bß║úng';
    ...
    enhanceMarkdownTables(container) {
        ...
        tables.forEach(table => {
            ...
            toolbar.innerHTML = `<span class="enhanced-table-info">${rows.length} h├áng ┬╖ ${headers.length} cß╗Öt</span>`;

            // Copy button
            const copyBtn = document.createElement('button');
            copyBtn.className = 'enhanced-table-btn';
            copyBtn.textContent = '≡ƒôï Copy';
            copyBtn.addEventListener('click', () => this.copyTableToClipboard(table, copyBtn));
            toolbar.appendChild(copyBtn);

            // Expand button ΓÇö open in full viewer
            if (headers.length > 0 && rows.length > 0) {
                const expandBtn = document.createElement('button');
                expandBtn.className = 'enhanced-table-btn';
                expandBtn.textContent = '≡ƒöì Xem ─æß║ºy ─æß╗º';
                expandBtn.addEventListener('click', () => {
                    csvPreview.show({ headers, rows }, 'Bß║úng tß╗½ AI');
                });
     ...
     *   - unordered lists (- / * / ΓÇó)
     *   - arrows (->  ΓåÆ  ==>  Γƒ╣)
     *   - blank lines ΓåÆ paragraph breaks
     */
    ...
    _normalisePaste(text) {
        return (text || '')
            ...
            .replace(/[\u200B-\u200D\uFEFF]/g, '')
            // Word "smart" quotes ΓåÆ standard
            .replace(/[\u2018\u2019]/g, "'")
            .replace(/[\u201C\u201D]/g, '"')
            // Word em/en dash ΓåÆ hyphen (preserve intent)
            .replace(/[\u2013\u2014]/g, '-')
            // Non-breaking spaces ΓåÆ regular space
            .replace(/\u00A0/g, ' ')
    ...
    _userTextToHtml(text) {
        ...
        while (i < lines.length) {
            ...
            // ΓöÇΓöÇ Fenced code block (``` or ~~~) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            ...
            // ΓöÇΓöÇ Blank line ΓåÆ paragraph break ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            if (raw.trim() === '') {
                out.push('<br>');
                i++;
                continue;
            ...
            // ΓöÇΓöÇ Detect list context ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            // Unordered: - / * / ΓÇó and first char after whitespace
            const ulMatch = raw.match(/^(\s*)([-*ΓÇó])\s+(.*)$/);
            if (ulMatch) {
                ...
                i++;
                while (i < lines.length) {
                    const next = lines[i].match(/^(\s*)([-*ΓÇó])\s+(.*)$/);
                    if (!next) break;
            ...
            // ΓöÇΓöÇ Regular line ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    _inlineFormat(escaped) {
        return escaped
            ...
            // Arrow variants: ->, =>, ==>, -->, ΓåÆ, Γƒ╣
            .replace(/(==&gt;|--&gt;|-&gt;|=&gt;|ΓåÆ|Γƒ╣|Γ₧£|Γ₧í)/g, '<span class="user-arrow">$1</span>');
    }
    ...
    createThinkingSection(thinkingProcess, isLoading = false) {
        ...
        if (isLoading) {
            pill.innerHTML = `
                ...
                <span class="thinking-pill__label">─Éang suy ngh─⌐</span>
            ...
            pill.innerHTML = `
                <span class="thinking-pill__done-icon">≡ƒºá</span>
                <span class="thinking-pill__label">─É├ú suy ngh─⌐ xong</span>
                <span class="thinking-pill__timer"></span>
                <span class="thinking-pill__chevron">ΓÇ║</span>
            `;
            if (thinkingProcess) {
    ...
    /**
     * Finalize thinking pill ΓÇö stop timer, update label to "─É├ú suy ngh─⌐ xong (Xs)".
     */
    finalizeThinking(container, data = {}) {
        ...
        // Rebuild inner HTML cleanly ΓÇö no surgical querySelector patching
        container.innerHTML = `
            <span class="thinking-pill__done-icon">≡ƒºá</span>
            <span class="thinking-pill__label">─É├ú suy ngh─⌐ xong</span>
    ...
    /**
     * Update thinking content (non-streaming fallback) ΓÇö populate pill from static data.
     */
    ...
    addResponseStats(contentDiv, stats = {}) {
        ...
        // ΓöÇΓöÇ Token circle gauge ΓöÇΓöÇ
    ...
    /**
     * Create action button ΓÇö uses Lucide icons for iOS bubble look
     */
    ...
    regenerateResponse(messageDiv) {
        // ΓöÇΓöÇ Image generation: detect and redirect ΓöÇΓöÇ
        ...
        window.chatApp.apiService.sendMessage(
            ...
            if (error.name === 'AbortError') {
                ...
                const errorMsg = window.chatApp.messageRenderer.addMessage(
                    chatContainer,
                    `Γ¥î **Lß╗ùi:** ${error.message || 'Kh├┤ng thß╗â tß║ío lß║íi response'}`,
                    false,
                    formValues.model,
                    formValues.context,
    ...
    /**
     * Regenerate image response ΓÇö re-runs image generation with same provider.
    ...
    async regenerateImageResponse(messageDiv) {
        ...
        if (provider === 'api' && regenCount >= 1) {
            alert('API chß╗ë cho ph├⌐p thß╗¡ lß║íi 1 lß║ºn. H├úy d├╣ng LOCAL ─æß╗â tß║ío kh├┤ng giß╗¢i hß║ín.');
            return;
        }
                ...
                <div class="message-content">
                    <div class="igv2-stream-status">
                        <div class="igv2-stream-header">
                            <span class="igv2-stream-icon spinning">ΓÜÖ∩╕Å</span>
        ...
        try {
                ...
                userPrompt, conversationId, window.chatApp.currentAbortController.signal,
                {
                    onStatus: (data) => {
                        if (data.phase === 'enhance') {
                            addStep(data.enhanced_prompt ? 'Γ£¿' : 'Γ£¿', data.enhanced_prompt ? 'Prompt enhanced' : data.step, data.enhanced_prompt ? 'done' : 'active');
                        } else if (data.phase === 'select') {
                            addStep(data.providers ? '≡ƒôí' : '≡ƒöì', data.providers ? `Providers: ${data.providers.join(', ')}` : data.step, data.providers ? 'done' : 'active');
                        } else {
                            addStep('ΓÜÖ∩╕Å', data.step, 'active');
                        }
                    },
                    onProviderTry: (data) => {
                        providerStep = addStep('≡ƒöä', `Trying ${data.provider} (${data.attempt}/${data.total_providers})...`, 'active');
                    },
                    onProviderFail: (data) => {
                        updateStep(providerStep, 'Γ¥î', `${data.provider} failed: ${data.error}`, 'fail');
                        providerStep = null;
                    },
                    onProviderSuccess: (data) => {
                        updateStep(providerStep, 'Γ£à', `${data.provider} / ${data.model} ΓÇö ${Math.round(data.latency_ms)}ms`, 'done');
                        headerIcon.textContent = 'Γ£à';
                        headerIcon.classList.remove('spinning');
                    },
                    onError: (data) => {
                        addStep('Γ¥î', data.error, 'fail');
                        headerIcon.textContent = 'Γ¥î';
                        headerIcon.classList.remove('spinning');
                    },
                },
                imageGenOptions,
            ...
            if (result.success) {
                ...
                const meta = `≡ƒÄ¿ **${result.provider}** / ${result.model} | ${Math.round(result.latency_ms)}ms | $${result.cost_usd}`;
                const enhanced = result.prompt_used ? `\n≡ƒô¥ ${result.prompt_used.substring(0, 150)}` : '';
                ...
                const overlayButtons = `
                    <div class="igv2-img-overlay">
                        <button type="button" class="igv2-img-btn" title="Tß║úi ß║únh" data-igv2-action="download" data-img-src="${imgSrcAttr}" data-image-id="${imageIdAttr}">Γ¼ç</button>
                        <button type="button" class="igv2-img-btn" title="Th├┤ng tin" data-igv2-action="info" data-image-id="${imageIdAttr}">Γä╣</button>
                        ${imageId ? `<button type="button" class="igv2-img-btn igv2-save-btn" title="L╞░u & Upload Drive" data-igv2-action="save" data-image-id="${imageIdAttr}">Γÿü</button>` : ''}
                    </div>`;
                ...
                const errContent = `Γ¥î Kh├┤ng thß╗â tß║ío ß║únh: ${result.error}`;
            ...
            headerIcon.textContent = 'Γ¥î';
            headerIcon.classList.remove('spinning');
            addStep('Γ¥î', error.message || 'Unknown error', 'fail');
        }
    }
    ...
    showMoreOptions(messageDiv, button) {
        ...
        menu.innerHTML = `
            <button class="option-item" data-action="copy">
                <span>≡ƒôï</span> Copy message
            </button>
            ${!isUser ? `
            <button class="option-item" data-action="export">
                <span>≡ƒôñ</span> Export to file
            </button>
            ` : ''}
            <button class="option-item" data-action="fork">
                <span>≡ƒöÇ</span> ${lang === 'vi' ? 'T├ích nh├ính tß╗½ ─æ├óy' : 'Fork from here'}
            </button>
            <button class="option-item" data-action="delete">
                <span>≡ƒùæ∩╕Å</span> Delete message
            </button>
        `;
    ...
    async copyMessageToClipboard(content, button) {
        ...
        try {
            ...
            if (button) {
                ...
                notification.textContent = 'Γ£à Copied to clipboard!';
                document.body.appendChild(notification);
                setTimeout(() => notification.remove(), 2000);
            }
        } catch (err) {
            console.error('Failed to copy:', err);
            alert('Kh├┤ng thß╗â copy. Vui l├▓ng thß╗¡ lß║íi!');
        }
    }
    ...
    async copyTableToClipboard(table, button) {
        ...
        try {
            ...
            button.textContent = 'Γ£à ─É├ú copy!';
            button.classList.add('copied');
            ...
            console.error('Failed to copy:', err);
            alert('Kh├┤ng thß╗â copy bß║úng. Vui l├▓ng thß╗¡ lß║íi.');
        }
    }
    ...
    showEditForm(messageDiv, originalContent, asTool = false) {
        ...
        // ΓöÇΓöÇ Rich Agent dropdown (thinkingMode-style) ΓöÇΓöÇ
        const agentMeta = [
            { value: 'casual',        icon: 'message-circle', label: 'Casual Chat',  desc: 'Tr├▓ chuyß╗çn tß╗▒ nhi├¬n', emoji: '≡ƒÆ¼' },
            { value: 'programming',   icon: 'code-2',         label: 'Programming',  desc: 'Hß╗ù trß╗ú lß║¡p tr├¼nh',    emoji: '≡ƒÆ╗' },
            { value: 'creative',      icon: 'palette',        label: 'Creative',     desc: 'S├íng tß║ío nß╗Öi dung',   emoji: '≡ƒÄ¿' },
            { value: 'research',      icon: 'search',         label: 'Research',     desc: 'Nghi├¬n cß╗⌐u chuy├¬n s├óu',emoji: '≡ƒö¼' },
            { value: 'psychological', icon: 'heart-handshake',label: 'Psychology',   desc: 'T├óm l├╜ & t╞░ vß║Ñn',     emoji: '≡ƒºÿ' },
            { value: 'lifestyle',     icon: 'sparkles',       label: 'Lifestyle',    desc: 'Phong c├ích sß╗æng',      emoji: '≡ƒîƒ' },
        ];
        ...
        textarea.placeholder = 'Chß╗ënh sß╗¡a tin nhß║»n...';
        ...
        saveBtn.textContent = '≡ƒÆ╛ L╞░u & Tß║ío lß║íi response';
        saveBtn.onclick = () => {
        ...
        cancelBtn.textContent = 'Γ¥î Hß╗ºy';
    ...
    navigateVersion(messageDiv, newIndex) {
        ...
        // ΓöÇΓöÇ Smooth user-message crossfade ΓöÇΓöÇ
        const textDiv = messageDiv.querySelector('.message-text');
        if (textDiv) {
        ...
        // ΓöÇΓöÇ Smooth assistant-message crossfade ΓöÇΓöÇ
        let assistantMsg = messageDiv.nextElementSibling;
        while (assistantMsg && !assistantMsg.classList.contains('assistant')) {
            assistantMsg = assistantMsg.nextElementSibling;
        }
                ...
                img.onload = () => {
                    previewInfo.innerHTML = `
                        <div class="lightbox__meta-grid">
                            <div class="lightbox__meta-item"><span class="lightbox__meta-label">Dimensions</span><span class="lightbox__meta-value">${img.width} ├ù ${img.height}</span></div>
                        </div>
                    `;
                };
