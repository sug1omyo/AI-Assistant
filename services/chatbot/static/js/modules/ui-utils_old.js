...
export class UIUtils {
     ...
     */
    initElements() {
        // Cache welcome screen permanently ΓÇö survives innerHTML clears
    ...
    initDarkMode() {
        ...
        if (savedTheme === 'light') {
            ...
            // Dark mode (default) ΓÇö no class needed
            if (this.elements.darkModeBtn && window.swapLucideIcon) {
    ...
    /**
     * Toggle dark mode
     * Cycles: dark ΓåÆ light ΓåÆ dark
     */
    toggleDarkMode() {
        ...
        // Toggle: dark (no class) Γåö light (light-mode class)
    ...
    toggleSidebar() {
        if (this.elements.sidebar) {
            ...
            if (toggleIcon) {
                toggleIcon.textContent = isCollapsed ? 'Γû╢' : 'ΓùÇ';
            }
    ...
    initSidebarState() {
        ...
        if (isCollapsed && this.elements.sidebar) {
            ...
            if (toggleIcon) toggleIcon.textContent = 'Γû╢';
        }
    }
        ...
        this.elements.chatList.innerHTML = sortedChats.map(id => {
            ...
            return `
                <div class="sidebar__chat-item ${isActive ? 'active' : ''} ${isPinned ? 'pinned' : ''}" 
                    ...
                    <span class="drag-handle" title="Drag to reorder">Γá┐</span>
            ...
            if (value.endsWith('-local')) {
                ...
                if (modelKey && modelsStatus[modelKey]) {
                    if (!modelsStatus[modelKey].available) {
                        ...
                        option.textContent += ' (Ch╞░a tß║úi)';
                    } else if (modelsStatus[modelKey].loaded) {
                        option.textContent = option.textContent.replace(' Γ¡É', '') + ' Γ£à';
                    }
                }
            }
        });
