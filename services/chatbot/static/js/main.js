...
class ChatBotApp {
    ...
    setupEventListeners() {
        ...
        // No-op — MCPController.setupSourceTabs() handles everything
    ...
    async sendMessage() {
        ...
        // 3. Image generation V2 route — early return
        if (isImageIntent) {
            return runImageRequestFlow(this, { message, formValues, elements, flushedFiles });
        }

        // 4. Img2Img route — early return if handled
        ...
        // 6. Streaming chat flow (SSE → fallback POST → finalize → suggestions)
    ...
    async saveCurrentSession(updateTimestamp = false) {
        ...
        // Structured data-first array — single source of truth for new sessions
...
// ── Image overlay delegation (runs at module load) ──────────────────────────
initOverlayActions();
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Export wrappers (keep temporarily — used by history modal buttons)
